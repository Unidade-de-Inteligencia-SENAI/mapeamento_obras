...
Saídas:
  - bronze: data/bronze_pncp_contratacoes.jsonl   (payload bruto, 1 JSON por linha)
  - silver: data/silver_pncp_contratacoes_obras.csv (tabela tratada, achatada)
...
# ── Ingestão bronze ──────────────────────────────────────────────────────
def ingest_pncp_bronze(anos: list, modo: str) -> list:
    """
    modo:
...
def main():
    ...
    if not args.skip_ingest:
        print("=== Bronze PNCP ===")
        ingest_pncp_bronze(args.anos, args.modo)
